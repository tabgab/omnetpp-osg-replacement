...
| `spike/CMakeLists.txt` | SPIKE | pre-M3 gate | **Rewritten during macOS bring-up:** Obj-C++ (`main.mm`), ARC, Metal/QuartzCore/AppKit frameworks; macOS-only (Linux uses the HgiGL path). | — |
| `spike/main.mm` | SPIKE | pre-M3 gate | **macOS gate PASSED (2026-06-11):** Metal-native present (native QWidget + CAMetalLayer sublayer) renders, picks, orbits stably against OpenUSD 25.11 + Qt 6.11.1 on Apple Silicon. Replaces the original `main.cpp` (GL-interop path — verified dead-end). Carries diag tooling (gated readback logs, 30-frame health log, `d`-key PPM dump) + the load-bearing synchronizing AOV readback (TODO M3: MTLSharedEvent fence). | Linux/HgiGL run; `--two` soak test |
| `spike/README.md` | SPIKE | pre-M3 gate | Updated with the validated macOS recipe, the 4 verified dead-ends, the load-bearing-readback finding, and remaining items (Linux run, overlay, fence). | — |
